...
public:
   ...
   vector<int> initNearestNeighbour(int n_points)
    ...
    vector<int> makeTransition(vector<int> list)
    {
        ...
        double r_q = ((double)rand() / (double)RAND_MAX);

        double cummulative_probs_current = 0;
        int selected_option = 0;
        int previus_selected_option = 0;
        ...
        for (int i = 0; i < options.size(); i++)
        {
            ...
            {
                previus_selected_option = i;
            }
        }

        if (r_q > Q)
        {
            selected_option = previus_selected_option;
        }
        ...
        // Fixed
        int new_city_for_ant = options[selected_option];

        list.push_back(new_city_for_ant);
        // Local update
        pheromones(current_city, new_city_for_ant) = (1 - rho) * pheromones(current_city, new_city_for_ant) + rho * tau0;
        pheromones(new_city_for_ant, current_city) = pheromones(current_city, new_city_for_ant);
    ...
    void swap3(vector<int> *list, int i, int j, int k, int best_case)
    {
        ...
        for (int i = 1; i < local_cities.size(); i++)
        {
            for (int j = i + 2; j < local_cities.size() - 1; j++)
            {
                for (int k = j + 2; k < local_cities.size() - 2 + (i > 0); k++)
                {
                    ...
                    {
                        swap3(&my_cities, i, j, k, best_case);
                    }
                }
            }
        }
    ...
    string stringifyVectorClassic(vector<int> v)
    {
        string s = "";
    ...
    double run()
    {
        ...
        // To return final
        double optimal_length = pathLength(nn);
        ...
        for (int i = 0; i < n_itter; i++)
        {
            ...
            // Initialize ants on random cities
            ...
            for (int c = 1; c < n_points; c++)
            {
                ...
                {
                    int seen = ants[a][c - 1];
                    vector<int> new_ants = makeTransition(ants[a]);
                    ants[a] = new_ants;
                }
            }
            ...
            for (int a = 0; a < n_ants; a++)
            {
                ants[a].push_back(ants[a][0]);
            }

            // best ant calculate
            int best_ant = -1;
            vector<int> best_ant_path;
            double best_ant_length = INFINITY;

            for (int a = 0; a < n_ants; a++)
            {
                double ant_length = pathLength(ants[a]);
                // Final local update
                int A = ants[a][0];
                int B = ants[a][n_points - 1];
                double local_evaporation = 0.33;
                pheromones(A, B) = (1 - local_evaporation) * pheromones(A, B) + local_evaporation * tau0;
                pheromones(B, A) = pheromones(A, B);

                if (ant_length < best_ant_length)
                {
                    best_ant = a;
                    best_ant_path = ants[a];
                    best_ant_length = ant_length;
                }
            }

            if (best_ant_length < optimal_length)
            {
                optimal_length = best_ant_length;
                optimal_path = best_ant_path;
                last_itter = i;
            }

            double random_or_best = (double)rand() / (double)RAND_MAX;

            vector<int> potentially_best = best_ant_path;
            int potentially_best_length = best_ant_length;

            if (random_or_best > 0.5)
            {
                int r = rand() % n_ants;
                vector<int> ant_path = ants[r];
                int ant_length = pathLength(ant_path);

                vector<int> new_cities = ants[r];
                int new_length;
                    ...
                    for (int i = 1; i < ant_path.size() - 2; i++)
                    {
                        for (int j = i + 1; j < ant_path.size() - 1; j++)
                        {
                            ...
                            if (diff < 0)
                            {
                                ...
                                if (new_length < ant_length)
                                {
                                    // cout << "Improvement: " << path_length << " -> " << new_length << endl;
                                    ant_path = new_cities;
                                    ant_length = new_length;
                                    swaps++;
                                }
                            }
                        }
                ...
                potentially_best = ant_path;
                potentially_best_length = ant_length;
            }
            else
            {
                vector<int> ant_path = ants[best_ant];
                int ant_length = pathLength(ant_path);

                vector<int> new_cities = ants[best_ant];
                int new_length;
            ...
            if (potentially_best_length < optimal_length)
            {
                optimal_length = potentially_best_length;
                optimal_path = potentially_best;
                last_itter = i;
            }

            // // update pheromone based on best ant

            for (int c = 0; c < n_points - 1; c++)
            {
                int from = optimal_path[c];
                int to = optimal_path[c + 1];
                pheromones(from, to) = (1 - rho) * pheromones(from, to) + rho * 1 / optimal_length;
                pheromones(to, from) = pheromones(from, to);
            }

            int from = optimal_path[0];
            int to = optimal_path[n_points - 1];
            pheromones(from, to) = (1 - rho) * pheromones(from, to) + rho * 1 / optimal_length;
            pheromones(to, from) = pheromones(from, to);
